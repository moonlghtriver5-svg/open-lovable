...
import { fileSearchExecutorV2, SearchPlan } from './file-search-executor-v2';
...
export class MultiPhaseReasoner {
  ...
  // Phase 2: v2's Precise File Search System  
  private async performContextSearch(
    ...
    v2StreamingManager.updateStatus(controller, 'Context Search', 
      `Executing precise file search for: ${intentAnalysis.searchTerms.join(', ')}`);

    // Create search plan based on intent analysis
    const searchPlan: SearchPlan = {
      editType: intentAnalysis.editType,
      reasoning: intentAnalysis.reasoning,
      searchTerms: intentAnalysis.searchTerms,
      regexPatterns: this.createRegexPatterns(intentAnalysis),
      fileTypesToSearch: this.getRelevantFileTypes(intentAnalysis.editType),
      expectedMatches: intentAnalysis.targetFiles.length || 1,
      fallbackSearch: {
        terms: this.createFallbackSearchTerms(intentAnalysis),
        patterns: this.createFallbackPatterns(intentAnalysis)
      }
    };

    // Execute precise search using v2's FileSearchExecutor
    const searchResult = fileSearchExecutorV2.executeSearchPlan(searchPlan, codebaseContext.files);
    
    if (!searchResult.success) {
      console.warn('[multi-phase-reasoner] File search failed:', searchResult.error);
      // Fallback to basic search
      return this.fallbackBasicSearch(intentAnalysis, codebaseContext);
    }

    // Format results for AI analysis
    const formattedResults = fileSearchExecutorV2.formatSearchResultsForAI(searchResult.results);
    
    // Select target file if needed
    const targetFile = fileSearchExecutorV2.selectTargetFile(searchResult.results, intentAnalysis.editType);

    console.log('[multi-phase-reasoner] Precise search completed:', {
    ...
    return {
      ...
      confidence: this.calculateSearchConfidence(searchResult.results, intentAnalysis)
    };
  }
...
Create a JSON plan with:
{
  ...
  // v2 File Search Helper Methods
  private createRegexPatterns(intentAnalysis: IntentAnalysis): string[] {
    const patterns: string[] = [];
    
    // Create patterns based on edit type
    switch (intentAnalysis.editType) {
      case 'UPDATE':
      case 'FIX':
        // Look for function/component definitions
        patterns.push(
          `(?:function|const|class)\\s+${intentAnalysis.searchTerms.join('|')}`,
          `${intentAnalysis.searchTerms.join('|')}\\s*[=:]`
        );
        break;
      case 'CREATE':
        // Look for import/export statements
        patterns.push(
          `(?:import|export).*${intentAnalysis.searchTerms.join('|')}`,
          `<${intentAnalysis.searchTerms.join('|')}`
        );
        break;
      case 'ENHANCE':
        // Look for component usage and props
        patterns.push(
          `<${intentAnalysis.searchTerms.join('|')}`,
          `${intentAnalysis.searchTerms.join('|')}\\s*\\{`
        );
  ...
  private getRelevantFileTypes(editType: string): string[] {
    switch (editType) {
      case 'UPDATE':
      case 'FIX':
      case 'ENHANCE':
  ...
  private createFallbackSearchTerms(intentAnalysis: IntentAnalysis): string[] {
    const fallbackTerms: string[] = [];
    
    // Add broader terms based on intent
    intentAnalysis.searchTerms.forEach(term => {
      // Add partial matches
      if (term.length > 4) {
        fallbackTerms.push(term.substring(0, term.length - 2));
      }
      
      // Add common variations
      if (term.includes('Component')) {
        fallbackTerms.push(term.replace('Component', ''));
      }
      if (term.includes('Button')) {
        fallbackTerms.push('btn', 'click');
      }
      if (term.includes('Form')) {
        fallbackTerms.push('input', 'submit');
      }
    });
    
    return [...new Set(fallbackTerms)]; // Remove duplicates
  }

  private createFallbackPatterns(intentAnalysis: IntentAnalysis): string[] {
    const patterns: string[] = [];
    
    // Generic component patterns
    patterns.push(
      '(?:function|const|class)\\s+\\w+Component',
      '<\\w+[^>]*>',
  ...
  private calculateSearchConfidence(searchResults: any[], intentAnalysis: IntentAnalysis): number {
    if (searchResults.length === 0) return 0;
    
    // Calculate confidence based on result quality
    const highConfidenceResults = searchResults.filter(r => r.confidence === 'high').length;
    const mediumConfidenceResults = searchResults.filter(r => r.confidence === 'medium').length;
    const totalResults = searchResults.length;
    
    // Weight high confidence results more heavily
    const weightedScore = (highConfidenceResults * 1.0) + (mediumConfidenceResults * 0.6) + ((totalResults - highConfidenceResults - mediumConfidenceResults) * 0.3);
    
    // Normalize to 0-1 scale
    const maxPossibleScore = totalResults * 1.0;
    return Math.min(weightedScore / maxPossibleScore, 1.0);
  }

  private async fallbackBasicSearch(intentAnalysis: IntentAnalysis, codebaseContext: CodebaseContext): Promise<any> {
    console.log('[multi-phase-reasoner] Using fallback basic search');
    ...
    return {
      searchResults: [],
      formattedResults: `Fallback search found ${searchResults.relevantFiles.length} relevant files`,
      targetFile: null,
      executionStats: {
        filesSearched: Object.keys(codebaseContext.files).length,
        executionTime: 0,
        usedFallback: true
      },
