...
async function handlePlanningMode(request: NextRequest, prompt: string, context: any, model: string) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        sendMessage({ type: 'status', message: `✅ Plan complete! Building with ${model}...` });
        
        // Stage 2: Building with selected model
        const buildResponse = await fetch(`${baseUrl}/api/generate-ai-code-stream`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
            prompt: prompt, // Use original prompt
            model: model, // Use the selected model for building too
            context: {
              ...context,
        ...
        if (Object.keys(generatedFiles).length > 0) {
          ...
          if (!(global as any).conversationState.context) {
            (global as any).conversationState.context = { currentFiles: {}, messages: [] };
          }
          if (!(global as any).conversationState.context.currentFiles) {
            (global as any).conversationState.context.currentFiles = {};
          }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (usePlanningMode) {
      return await handlePlanningMode(request, prompt, context, model);
    }
...
🚨 PROGRESS BAR CRITICAL REQUIREMENTS:
**EVERY progress bar MUST calculate correctly - this is NON-NEGOTIABLE**

✅ CORRECT CALCULATION (ALWAYS USE THIS):
const completedCount = todos.filter(todo => todo.completed).length;
const totalCount = todos.length;
const progress = totalCount > 0 ? (completedCount / totalCount) * 100 : 0;

❌ NEVER DO THIS (COMMON MISTAKES):
- hardcoded: const progress = 100;
- wrong formula: const progress = todos.length * 100;
- missing percentage: const progress = completedCount;
- forgetting edge cases (division by zero)

🎯 PROGRESS BAR IMPLEMENTATION CHECKLIST:
1. Calculate completedCount using filter()
2. Get totalCount using array.length
3. Handle edge case: totalCount === 0
4. Calculate percentage: (completed/total) * 100
5. Use inline style for width: style={{width: progress + '%'}}
6. Display text: "X of Y tasks completed (Z%)"
7. Test with sample data immediately
